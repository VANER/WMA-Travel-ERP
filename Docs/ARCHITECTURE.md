...
## 3.2.1 Schemas Reservados (Não Populados)

Três schemas do banco existem fisicamente, mas ainda não recebem tabelas: `logs`,
`seguranca` e `util`. Essa é uma decisão de arquitetura registrada, não uma
pendência esquecida.

**Situação atual (2026-08-09):** as tabelas que logicamente pertencem a esses
schemas ainda estão hospedadas em `public.*`, junto com o restante do módulo
CORE:

```text
Candidatas ao schema logs
├── public.log_api
...
Candidatas ao schema seguranca
...
Candidatas ao schema util
├── public.configuracao_empresa
└── public.parametro_sistema
```

**Por que a migração foi adiada:** `public.log_auditoria` é referenciada
diretamente (schema fixo) pela função `fn_log_auditoria()`, que já está
